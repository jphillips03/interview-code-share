...
A direct UDP connection is attempted for code sharing through an open relay STUN server. Network restrictions may cause direct p2p connections to fail. In these cases a TURN server is as a fallback to handle streaming connection data between peers. When utilized the TURN server routes packets securely through pre-encrypted DTLS-SRTP protocols, it cannot decrypt or view any data exchanged between peers. The publicly available open relay TURN servers are used by default for this. In order to use the TURN server the interviewer must enter username/credential information on the main config page when starting an interview. Open relay offers free account options, with more details available at [https://www.metered.ca/](https://www.metered.ca/), and signup options at [https://dashboard.metered.ca/signup](https://dashboard.metered.ca/signup).
